...
export function ShadowScale({ isLight }: { isLight: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-5 h-full overflow-y-auto">
      {/* Header */}
      <div>
        <h2 className="text-lg font-semibold mb-0.5" style={{ color: "var(--text)", fontFamily: "var(--font-serif)", fontStyle: "italic" }}>
          Elevation Scale
        </h2>
      ...
      >
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-semibold tracking-[0.08em] uppercase" style={{ color: "var(--text-muted)" }}>Color</label>
            <div className="flex items-center gap-2">
              <input
                ...
                style={{ border: "2px solid var(--border-hover)", background: "none", padding: 2 }}
              />
              <span className="text-xs font-mono" style={{ color: "var(--text-muted)" }}>{color.toUpperCase()}</span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1.5 flex-1 min-w-32">
            <div className="flex justify-between">
              <label className="text-[11px] font-semibold tracking-[0.08em] uppercase" style={{ color: "var(--text-muted)" }}>Base Opacity</label>
              <span className="text-[11px] font-mono" style={{ color: "var(--accent)", background: "var(--surface-raised)", padding: "1px 6px", borderRadius: 4 }}>{intensity}%</span>
            </div>
            <input
              type="range"
              min={1} max={30} value={intensity}
              ...
              style={{ accentColor: "var(--accent)", background: `linear-gradient(to right, var(--accent) ${intensity / 30 * 100}%, var(--surface-raised) 0%)` }}
            />
          </div>

          {/* Style */}
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-semibold tracking-[0.08em] uppercase" style={{ color: "var(--text-muted)" }}>Style</label>
            <div className="flex items-center gap-1 p-0.5 rounded-xl" style={{ background: "rgba(128,128,128,0.08)", border: "1px solid var(--border)" }}>
              {STYLES.map(s => (
                <button
                  key={s.id}
                  ...
                  style={{
                    background: style === s.id ? "var(--surface-raised)" : "transparent",
                    color: style === s.id ? "var(--text)" : "var(--text-muted)",
                    border: style === s.id ? "1px solid var(--border-hover)" : "1px solid transparent",
                  }}
                >
                  {s.label}
                </button>
      ...
      <div
        ...
        style={{ background: "var(--surface)", border: "1px solid var(--border)" }}
      >
        <p className="text-[11px] font-semibold tracking-[0.08em] uppercase" style={{ color: "var(--text-muted)" }}>Preview</p>
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
          {scale.map(entry => (
            <div key={entry.name} className="flex flex-col items-center gap-2.5">
              <div
                className="w-full rounded-xl flex items-center justify-center"
                style={{ background: canvasBg, height: 72, transition: "background 0.2s ease" }}
              >
                <div
              ...
              <span className="text-[11px] font-mono font-semibold" style={{ color: "var(--text-muted)" }}>{entry.name}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        >
          <div className="flex items-center overflow-x-auto" style={{ scrollbarWidth: "none" }}>
            {EXPORTS.map(e => (
              <button
                ...
                style={{ color: exportType === e.id ? "var(--accent)" : "var(--text-muted)" }}
              >
                {e.label}
                {exportType === e.id && (
          ...
          <div className="px-2 py-1.5 shrink-0">
            <button
              ...
              style={{
                background: copied ? "rgba(94,158,136,0.15)" : "rgba(128,128,128,0.1)",
                color: copied ? "var(--accent)" : "var(--text-muted)",
                border: `1px solid ${copied ? "rgba(94,158,136,0.3)" : "var(--border)"}`,
              }}
            >
              {copied ? <Check size={11} className="animate-check-pop" /> : <Copy size={11} />}
              {copied ? "Copied!" : "Copy All"}
            </button>
          </div>
        </div>
        <div style={{ background: "#0a1010" }}>
          <div className="flex items-center gap-1.5 px-4 pt-3">
            <div className="w-2.5 h-2.5 rounded-full" style={{ background: "#c96060", opacity: 0.8 }} />
            <div className="w-2.5 h-2.5 rounded-full" style={{ background: "#b08040", opacity: 0.8 }} />
            <div className="w-2.5 h-2.5 rounded-full" style={{ background: "#5e9e88", opacity: 0.8 }} />
          </div>
          <pre className="p-4 text-xs font-mono leading-relaxed overflow-x-auto whitespace-pre-wrap" style={{ color: "#8ecfb8", maxHeight: 200, overflowY: "auto" }}>
            {getExportCode()}
          </pre>
        </div>
      </div>
