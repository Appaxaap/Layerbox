...
export type ExportFormat = "css" | "tailwind" | "tailwind-config" | "scss" | "css-var" | "js" | "flutter"
...
export function toFlutter(shadows: Shadow[]): string {
  ...
  const layers = shadows.map(s => {
    ...
    if (s.inset) lines.push(`    // Note: Flutter does not support inset shadows natively`)
    lines.push(`  ),`)
    return lines.join("\n")
  })
...
export function getFormatCode(format: ExportFormat, shadows: Shadow[]): string {
  switch (format) {
    case "css":             return toCss(shadows)
    case "tailwind":        return toTailwind(shadows)
    case "tailwind-config": return toTailwindConfig(shadows)
    case "scss":            return toScss(shadows)
    case "css-var":         return toCssVar(shadows)
    case "js":              return toJs(shadows)
    case "flutter":         return toFlutter(shadows)
  }
}
