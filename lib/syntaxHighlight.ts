...
const KEYWORDS = new Set([
  ...
  "import", "export", "return", "new", "class", "extends",
  "static", "async", "await", "true", "false", "null",
  "undefined", "if", "else", "for", "of", "in", "from",
  "require", "this", "super", "throw", "try", "catch", "finally",
  "switch", "case", "default", "break", "continue", "while", "do",
...
function tokenize(code: string): Token[] {
  ...
  while (i < code.length) {
    ...
    // Hex color
    if (code[i] === "#" && i + 1 < code.length && /[0-9a-fA-F]/.test(code[i + 1])) {
      let j = i + 1
    ...
    if (/[A-Z]/.test(code[i]) && i + 1 < code.length && /[a-zA-Z]/.test(code[i + 1])) {
      let j = i + 1
    ...
    if (/\d/.test(code[i]) || (code[i] === "-" && i + 1 < code.length && /\d/.test(code[i + 1]))) {
      ...
      while (j < code.length && /[\d.eE]/.test(code[j])) {
        if (code[j] === ".") { if (hasDot) break; hasDot = true }
        if (code[j] === "e" || code[j] === "E") { j++; if (j < code.length && /[+-]/.test(code[j])) j++; break }
        j++
      }
      // Optional CSS unit
